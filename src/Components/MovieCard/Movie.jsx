  ...
  return (
    <>
      {data?.length > 0 ? (data.map((e, i) => (
        <Link to={`/${forTv ? forTv.toLowerCase() : 'movies'}/${search || toprated || upcoming || discover || similar || now_playing ? search || toprated || upcoming || discover || similar || now_playing : ''}/${e.id}`} key={e.id} style={{ color: "#000", textDecoration: 'none' }}>
